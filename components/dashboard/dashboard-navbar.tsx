...
export function DashboardNavbar({ title = "Dashboard" }: DashboardNavbarProps) {
  ...
  return (
    <div className="flex flex-row items-center justify-between gap-4 p-4 pl-16 md:p-6 bg-[#F8F7F4]/50 backdrop-blur-sm sticky top-0 z-30 mb-2 transition-all">
      <div className="flex flex-col gap-1">
        <p className="text-sm font-medium text-[#707EAE] hidden sm:block">Pages / {title}</p>
        <h1 className="text-xl md:text-3xl font-bold text-[#1C1917] tracking-tight">{title}</h1>
      </div>
      ...
      <div className="flex items-center gap-3 bg-white p-3 rounded-full shadow-[px_18px_40px_rgba(112,144,176,0.12)] w-fit">
        ...
        {/* <div className="relative hidden lg:block bg-[#F8F7F4] rounded-full">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <Search className="h-4 w-4 text-[#1C1917]" />
          </div>
          <input
            type="text"
            className="block w-full pl-10 pr-3 py-2 border-none rounded-full leading-5 bg-transparent text-[#1C1917] placeholder-[#8F9BBA] focus:outline-none focus:ring-0 sm:text-sm transition-colors w-40 focus:w-60 transition-all"
            placeholder="Search..."
          />
        </div> */}
        ...
        {/* <div className="flex items-center gap-1">
            <Button variant="ghost" size="icon" className="text-[#78716C] hover:text-[#1C1917] hover:bg-transparent rounded-full">
