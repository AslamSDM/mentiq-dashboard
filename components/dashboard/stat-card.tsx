      ...
      <CardContent className="p-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <h3 className="text-2xl font-bold text-[#1C1917] mt-1">{value}</h3>
          </div>
          {icon && (
            <div className={cn("p-3 rounded-full flex items-center justify-center", iconBg || "bg-[#F8F7F4]")}>
              {icon}
            </div>
          )}
        </div>
          ...
          <div className="flex items-center gap-2 mt-4 text-sm">
            <span
              className={cn(
                ...
                isPositive ? "text-[#16A34A]" : "text-[#EE5D50]"
              )}
            >
              {isPositive ? <ArrowUpIcon className="h-4 w-4 mr-1" /> : <ArrowDownIcon className="h-4 w-4 mr-1" />}
              {Math.abs(trend)}%
            </span>
            <span className="text-[#78716C] text-xs">{trendLabel || "since last month"}</span>
          </div>
        )}
      </CardContent>
    </Card>
