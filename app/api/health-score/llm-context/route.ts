  ...
  try {
    ...
    if (body.churnData || body.featureData || body.sessionData) {
        ...
        adoption: {
          coreFeatures: body.featureData?.features
            ? {
                ...
                used: body.featureData.features.filter(
                  (f: any) => f.adoption_rate > 0
                ).length,
              }
            : undefined,
          adoptionRate:
            body.featureData?.features?.length > 0
              ? body.featureData.features.reduce(
                  (sum: number, f: any) => sum + (f.adoption_rate || 0),
                  0
                ) / body.featureData.features.length
              : undefined,
          featureDepth:
            body.featureData?.features?.length > 0
              ? body.featureData.features.reduce(
      ...
      return NextResponse.json(
        {
          error:
            "Invalid input format. Provide either raw analytics data or HealthScoreInputs.",
        },
        { status: 400 }
      );
    }
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    ...
    usage: {
      ...
      description:
        "Calculate health score and get LLM-ready context for AI-powered churn reduction",
    },
